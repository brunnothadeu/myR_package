...
vetor.estendido <-
  function(dados, bib){
    if(sum(dados$CAD %in% names(bib)) != nrow(dados)) stop("Há cadernos não presentes no BIB")
    if(length(unique(unlist(lapply(bib, length)))) > 1) stop("Não implementado para cadernos com diferentes números de blocos")
    b.len <- (ncol(dados) - 3) / bib %>% lapply(length) %>% unlist %>% unique
    nblocos <- bib %>% unlist %>% unique %>% length
    aux <- matrix(NA, nrow = nrow(dados), ncol = b.len * nblocos) %>% data.frame
    for(cad in names(bib)){
      ind <-  bib[[cad]] %>% subtract(1) %>% multiply_by(b.len - 1) %>% add(bib[[cad]])
      ind <- ind %>% lapply(FUN = function(x, b.len){seq(x, x + b.len - 1)}, b.len = b.len) %>% unlist
      aux[dados$CAD == cad, ind] <- dados[4:ncol(dados)] %>% subset(dados$CAD == cad)
    }
    aux <- dados[1:3] %>% cbind(aux); names(aux) = c("ID", "CAD", "PESO", paste0("IT", 1:(nblocos * b.len)))
    return(aux)
  }
...
vetor.estendido2 <-
  function(dados, configCADS){
    ...
    CodIts <- CodIts[CodIts %>% order(decreasing = F)]
    aux <- matrix(NA, nrow = nrow(dados), ncol = length(CodIts)) %>% data.frame
    names(aux) <- paste0("IT_", CodIts)
    for(cad in names(configCADS))
      aux[dados$CAD == cad, paste0("IT_", configCADS[[cad]])] <- dados[4:ncol(dados)] %>% subset(dados$CAD == cad)
    aux <- dados[1:3] %>% cbind(aux)
    return(aux)
  }

corrigir <-
  function(dados, gab){
    dadosCorr <- dados[1:3] %>% cbind(dados[-c(1:3)] %>% key2binary(gab))
...
tct <-
  function(dados, dadosCorr, mapa, Alts = c("A", "B", "C", "D", "E", "*", "."), pbis = FALSE, itemResto = FALSE, arred = 10, sdpop = FALSE, crit = c(0.16, 0.1, 0), summary = FALSE){
    ...
    alphaResto <- psych::alpha(dadosCorr[c(rep(F, 3), hSD, F)])
    TCT$Alpha <- NA
...
agi.plot <-
  function(dados, desemp, redGab = NULL, main = "AGI PLOT", files = NULL, xlab = "Escore", ylab = "Percentual de Respostas", Alts = c(".", "*", "A", "B", "C", "D", "E"), col.agi = NULL, cex = c(3, 1.2, rep(0.7, 5)), intervals = 50, density = TRUE, smooth = NULL, dir.create = "AGI", xlim = NULL, width = 9.6, height = 6.8, ext = "PDF", shinyDemo = NULL){
    is.prof <- FALSE
    if(length(unique(desemp)) > intervals){
      desemp %<>% cut_interval(intervals) %>% as.character %>% substr(., 2, nchar(.)-1) %>% strsplit(",") %>% lapply(FUN = function(x) mean(as.numeric(x))) %>% unlist
      if(xlab == "Escore") xlab <- "Proficiência"
      is.prof <- TRUE
    }
    if(is.null(col.agi))   col.agi <- Alts %>% length %>% distinctColorPalette
    if(is.null(xlim))      xlim <- c(min(desemp), max(desemp))
    if(length(main) == 1)  main %<>% rep(ncol(dados))
    if(is.null(shinyDemo)) dir.create %>% dir.create(showWarnings = FALSE)
    for(i in seq_along(dados)){
      ...
      freq <- table(desemp, dados[[i]]) %>% prop.table(margin = 1)
      freq[is.na(freq)] <- 0
...
read.SCO <-
  function(SCO, summary = TRUE, getID = FALSE){
    if(length(SCO) == 1) SCO <- readLines(paste0(SCO, ".SCO"))
    SCO <- SCO[-c(1,2)]
    n <- SCO %>% length
...
checkDIF <-
  function(EXP, SCO, newGroup, crit = .15, probs = c(.05, .95)){
    ...
    for(i in seq_along(EXP)){
      ...
      for(g in unique(temp$Grupo)){
        for(gNew in newGroup){
          AUX <- subset(temp, temp$Grupo == g & temp$POINT >= max(SCO[[g]][1], SCO[[gNew]][1]) & temp$POINT <= min(SCO[[g]][2], SCO[[gNew]][2]))
        ...
        AUX <- subset(temp, temp$Grupo == g & temp$POINT >= SCO[[g]][1] & temp$POINT <= SCO[[g]][2])
    ...
    DIF$Ponto <- DIF$Ponto %>% as.numeric; DIF$DIF <- DIF$DIF %>% as.numeric
    Ajuste <- data.frame(Ajuste[-1, ], stringsAsFactors = F); names(Ajuste) <- c("Item", "Grupo", "Ponto", "DIF")
    Ajuste$Ponto <- Ajuste$Ponto %>% as.numeric; Ajuste$DIF <- Ajuste$DIF %>% as.numeric
...
dif.plot <-
  function(EXP, SCO, main = NULL, groups = NULL, probs = c(.05, .95), col.dif = NULL, density = "area", dir.create = "DIF", xlim = c(-4, 4), width = 9.6, height = 6.8, shinyDemo = NULL){
    if(is.null(groups))    groups <- paste0("Grupo", unique(SCO$Grupo))
    if(is.null(col.dif))   col.dif <- groups %>% length %>% distinctColorPalette
    if(is.null(shinyDemo)) dir.create %>% dir.create(showWarnings = FALSE)
    if(is.null(main))      main <- names(EXP)
    if(length(main) < length(EXP) & is.null(shinyDemo)) stop("Dimensões inválidas para main")
    if(!is.null(density)){
      habs <- SCO
      if(density == "points"){
        habs <- split(habs$SCO, habs$Grupo)
        for(i in seq_along(habs))
          habs[[i]] <- habs[[i]] %>% density
      }
    }
    SCO <- split(SCO$SCO, SCO$Grupo) %>% lapply(quantile, probs = c(probs[1], probs[2]))
    for(i in seq_along(EXP)){
      if(is.null(shinyDemo)){
...
checkDIF2 <-
  function(EXP, SCO, newGroup, crit = .15, minresp = 20, probs = c(.05, .95)){
    quants <- split(SCO$SCO, SCO$Grupo) %>% lapply(quantile, probs = c(probs[1], probs[2]))
    DIF <- matrix(NA, ncol = 5)
    for(i in seq_along(EXP)){
      temp <- EXP[[i]]
      for(gNew in newGroup){
        if(gNew %in% temp$Grupo){
          x <- temp[temp$Grupo == gNew, "POINT"] %>% unique
          dif <- (x[-1] - x[-length(x)]) / 2
          max <- x + c(dif, mean(dif))
          zNew <- cut(SCO$SCO[SCO$Grupo == gNew], breaks = c(x[1] - mean(dif), max), labels = x) %>% table %>% data.frame
          zNew$Var1 <- x
          for(g in unique(temp$Grupo)){
            AUX <- subset(temp, temp$Grupo == g & temp$POINT >= max(quants[[g]][1], quants[[gNew]][1]) & temp$POINT <= min(quants[[g]][2], quants[[gNew]][2]))
            x <- temp[temp$Grupo == g, "POINT"] %>% unique
            dif <- (x[-1] - x[-length(x)]) / 2
            max <- x + c(dif, mean(dif))
            z <- cut(SCO$SCO[SCO$Grupo == g], breaks = c(x[1] - mean(dif), max), labels = x) %>% table %>% data.frame
            z$Var1 <- x
            AUX <- subset(AUX, AUX$POINT %in% c(zNew$Var1[zNew$Freq > minresp], z$Var1[z$Freq > minresp]))
            if(max(c(AUX[[paste0("DIF", gNew)]], 0), na.rm = T) >= crit)
              DIF <- rbind(DIF, c(AUX$Item[1], c(g, gNew)[order(c(g, gNew))], AUX$POINT[AUX[[paste0("DIF", gNew)]] == max(AUX[[paste0("DIF", gNew)]], na.rm = T)], max(AUX[[paste0("DIF", gNew)]])))
          }
        }
...
full.plot <- 
  function(dados, desemp, pars, EXP, SCO, xlab = c("Escore", "Proficiência", "Proficiência"), ylab = c("Percentual de Respostas", "Probabilidade", "Percentual de Respostas"), main = "ITEM PLOT",
           files = NULL, redGab = NULL, xlim.agi = NULL, smooth = NULL, Alts = c(".", "*", "A", "B", "C", "D", "E"), col.agi = NULL, cex = c(3, 1.2, rep(0.7, 5)), intervals = 50,
           breaks = c(-4, 4, 0.1), groups = NULL, probs = c(0.05, 0.95), col.dif = NULL, xlim.dif = c(-4, 4), dir.create = "Full Plot", width = 9.6, height = 6.8, ext = "PDF", shinyDemo = NULL){
    if(ncol(dados) != length(EXP) | ncol(dados) != nrow(pars)) stop("O número de colunas em dados não confere com o número de linhas em pars e extensão em EXP")
    if(length(main) == 1) main %<>% rep(ncol(dados))
    if(length(unique(desemp)) > intervals){
      desemp %<>% cut_interval(intervals) %>% as.character %>% substr(., 2, nchar(.)-1) %>% strsplit(",") %>% lapply(FUN = function(x) mean(as.numeric(x))) %>% unlist
    ...
    for(i in seq_along(EXP)){
      if(is.null(shinyDemo)){
        if(ext == "PNG"){
          png(file = paste0(getwd(), "/",dir.create,"/", paste0(ifelse(is.null(files), pars$Codigo[i], files[i]), ".png")), width = width, height = height, res = 100)
        }else{
          pdf(file = paste0(getwd(), "/",dir.create,"/", paste0(ifelse(is.null(files), pars$Codigo[i], files[i]), ".pdf")), width = width, height = height, paper = "USr")
        }
      }else{
        i <- shinyDemo
      }
...
makedb <-
  function(dados){
    dados <- data.frame(ID = 1:nrow(dados), CAD = 1, PESO = 1, dados, stringsAsFactors = F)
    dados <- data.frame(sapply(dados, as.character), stringsAsFactors = F)
